...
if (!in_array('factions', $site_sections[$section_key]['modules'], true)) {
    trigger_error('Faction management is not enabled for this section.');
}

$section_name = $site_sections[$section_key]['name'];

$form_key = 'admin_factions';
...
function admin_faction_slug(string $value): string
{
    $value = strtolower(trim($value));
    $value = preg_replace('/[^a-z0-9]+/i', '-', $value);
    $value = trim((string) $value, '-');

    return $value !== '' ? $value : 'faction-entry';
}

function admin_redirect_factions(string $section_key): void
{
    redirect('/admin/?page=factions&section=' . rawurlencode($section_key));
}
...
if ($request->is_set_post('save')) {
    if (!check_form_key($form_key)) {
        ...
        if ($slug === '') {
            $slug = admin_faction_slug($name);
        } else {
            $slug = admin_faction_slug($slug);
        }

        if ($image_url !== '' && !filter_var($image_url, FILTER_VALIDATE_URL) && str_starts_with($image_url, '/') === false) {
            $error = 'Image must be a valid URL or local path beginning with /.';
        }
        ...
        if ($error === '') {
            if ($id > 0) {
                $sql = 'UPDATE site_factions SET ' . $db->sql_build_array('UPDATE', [
                    'name' => $name,
                    'slug' => $slug,
                    'image_url' => $image_url,
                ...
                $sql = 'INSERT INTO site_factions ' . $db->sql_build_array('INSERT', [
...
if ($action === 'delete' && $id > 0) {
    if (!check_link_hash($request->variable('hash', ''), 'admin_factions_delete_' . $id)) {
    ...
    $db->sql_query($sql);
    admin_redirect_factions($section_key);
}

if ($action === 'restore' && $id > 0) {
    if (!check_link_hash($request->variable('hash', ''), 'admin_factions_restore_' . $id)) {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-1">Manage Factions</h1>
...
<div class="card mb-4">
    <div class="card-header">
        <?= ((int) $edit_entry['id'] > 0) ? 'Edit Faction Entry' : 'Add Faction Entry' ?>
    </div>

    <div class="card-body">
        <form method="post" action="/admin/?page=factions&amp;section=<?= admin_h($section_key) ?>">
<?= build_hidden_fields([
...
<?= admin_form_token($form_key) ?>
            ...
            <div class="mb-3">
                ...
                <input
                    ...
                    maxlength="500"
                    placeholder="/images/wow/factions/example.jpg or https://cdn.example.com/example.jpg"
                >
            </div>
            ...
            <?php if (!empty($edit_entry['image_url'])): ?>
                <div class="mb-3">
                    <img
                        src="<?= admin_h($edit_entry['image_url']) ?>"
                        alt=""
                        class="img-thumbnail"
            ...
            <?php if ((int) $edit_entry['id'] > 0): ?>
                <a class="btn btn-outline-secondary" href="/admin/?page=factions&amp;section=<?= admin_h($section_key) ?>">
                    Cancel Edit
                </a>
            <?php endif; ?>
        </form>
        ...
        <table class="table table-striped table-hover align-middle mb-0">
            ...
            <tbody>
                <?php if (!$entries): ?>
                    <tr>
                        <td colspan="6" class="text-center text-secondary py-4">
                            No faction entries have been added yet.
                        </td>
                    </tr>
                <?php endif; ?>
                ...
                <?php foreach ($entries as $entry): ?>
                    ...
                    $delete_hash = generate_link_hash('admin_factions_delete_' . $entry_id);
                    $restore_hash = generate_link_hash('admin_factions_restore_' . $entry_id);
                    ?>
                    <tr class="<?= ((int) $entry['is_active'] === 0) ? 'table-secondary opacity-75' : '' ?>">
                        ...
                        <td>
                            <a
                                ...
                                href="/admin/?page=factions&amp;section=<?= admin_h($section_key) ?>&amp;action=edit&amp;id=<?= $entry_id ?>"
                            >
                                Edit
                            </a>
                            ...
                            <?php if ((int) $entry['is_active'] === 1): ?>
                                <a
                                    ...
                                    href="/admin/?page=factions&amp;section=<?= admin_h($section_key) ?>&amp;action=delete&amp;id=<?= $entry_id ?>&amp;hash=<?= admin_h($delete_hash) ?>"
                                    onclick="return confirm('Disable this faction entry?');"
                                >
                                    Disable
                                </a>
                            <?php else: ?>
                                <a
                                    ...
                                    href="/admin/?page=factions&amp;section=<?= admin_h($section_key) ?>&amp;action=restore&amp;id=<?= $entry_id ?>&amp;hash=<?= admin_h($restore_hash) ?>"
                                >
                                    Restore
                                </a>
                            <?php endif; ?>
